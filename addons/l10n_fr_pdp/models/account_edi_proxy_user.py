...
class AccountEdiProxyClientUser(models.Model):
    ...
    proxy_type = fields.Selection(selection_add=[('pdp', 'PA')], ondelete={'pdp': 'cascade'})
            ...
            if record_model == 'pdp.response':
                ...
                if content.get('error'):
                    if content['error'].get('code') == 702:
                        ...
                        # thrown when the IAP is still processing the message
                        continue
                    move._message_log(body=self.env._("French e-invoicing error: %s", content['error'].get('data', {}).get('message') or content['error']['message']))
                    move.peppol_move_state = 'error'
                    processed_message_uuids.append(uuid)
                    continue

                move.peppol_move_state = content['state']
                move._message_log(body=self.env._('French e-invoicing status update: %s', content['state']))
                processed_message_uuids.append(uuid)
        ...
        for uuid, content in messages.items():
            ...
            if flow_number == '1':
                ...
                if not origin_uuid or not origin_move:
                    ...
                    continue
                if response := self._pdp_import_mandatory_status_lifecycle(uuid, content, origin_move[:1]):
                    processed_messages[uuid] = response

        return processed_messages

    def _pdp_import_tax_extract_lifecycle(self, uuid, content, origin_move):
        ...
        if not ppf_state or response_code not in response_code_to_description_map or not issue_date:
            origin_move._message_log(
                body=self.env._(
                    "[Flow 1] Failed to process incoming response (Response Code = %(response_code)s; Issue Date = %(issue_date)s).%(br)s%(status_info)s",
        ...
        if content['state'] == 'done':
            origin_move._message_log(
                body=self.env._(
                    "[Flow 1] Received response with Response Code '%(response_code)s' issued on %(issue_date)s.%(br)s%(status_info)s",
                    response_code=response_code_description,
                    issue_date=issue_date,
                    status_info=markup_status_info,
                    br=Markup('<br/>It included the following status info:<br/>') if markup_status_info else '',
    ...
    def _pdp_import_mandatory_status_lifecycle(self, uuid, content, origin_move):
        response = self.env['pdp.response']
        if not origin_move:
            return response
        ...
        enc_key = content["enc_key"]
        document_content = content["document"]
        origin_ref_status_code = content["origin_ref_status_code"]
        ...
        ppf_state = RESPONSE_CODE_TO_PPF_STATE.get(response_code)
        if not ppf_state or response_code not in response_code_to_description_map or not issue_date or not ref_status_code_description:
            origin_move._message_log(
                body=self.env._(
                    "[Flow 6] Failed to process incoming response for status '%(ref_status)s' (Response Code = %(response_code)s; Issue Date = %(issue_date)s).%(br)s%(status_info)s",
                    ref_status=ref_status_code_description or origin_ref_status_code,
                    response_code=response_code,
                    issue_date=issue_date,
                    status_info=markup_status_info,
                    br=Markup('<br/>It included the following status info:<br/>') if markup_status_info else '',
        ...
        response = self.env['pdp.response'].create({
            'peppol_message_uuid': uuid,
            'response_code': PROCESS_CONDITION_CODE_TO_RESPONSE_CODE.get(origin_ref_status_code),
            'pdp_state': content['state'],
            'ppf_state': ppf_state,
            'move_id': origin_move.id,
            'status_info': '\n\n'.join([self._format_status_dict(status, separator=Markup('\n')) for status in status_list]),
        ...
        if content['state'] == 'done':
            origin_move._message_log(
                body=self.env._(
                    "[Flow 6] Received response for status '%(ref_status)s' with Response Code '%(response_code)s' issued on %(issue_date)s.%(br)s%(status_info)s",
                    ref_status=ref_status_code_description,
                    response_code=response_code_description,
                    issue_date=issue_date,
                    status_info=markup_status_info,
                    br=Markup('<br/>It included the following status info:<br/>') if markup_status_info else '',
    ...
    def _pdp_import_response(self, uuid, content, origin_move):
        ...
        if response_code not in response_code_to_description_map or not issue_date:
            origin_move._message_log(
                body=self.env._(
                    "Failed to process incoming response (Response Code = %(response_code)s; Issue Date = %(issue_date)s).%(br)s%(status_info)s",
                    response_code=response_code,
                    issue_date=issue_date,
                    status_info=markup_status_info,
                    br=Markup('<br/>It included the following status info:<br/>') if markup_status_info else '',
    ...
    def _pdp_import_invoice(self, uuid, content, journal):
        if not journal:
            _logger.warning('The French e-invoicing document with UUID %s could not be imported (missing journal)', uuid)
