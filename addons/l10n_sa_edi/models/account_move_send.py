...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        edi_moves = moves.filtered(lambda move: move._l10n_sa_is_phase_2_applicable())
        ...
        suppliers = edi_moves.company_id.partner_id.commercial_partner_id.filtered(lambda rec: not rec.city or not rec.state_id)
        missing_supplier_fields = []
        if not all(supplier.city for supplier in suppliers):
            missing_supplier_fields.append('City')
        if not all(supplier.state_id for supplier in suppliers):
            missing_supplier_fields.append('State')
        if missing_supplier_fields:
            res['l10n_sa_edi_supplier_missing'] = {
                'message': _("Please the set the following on the company: %s", ', '.join(missing_supplier_fields)),
                'level': 'danger',
                'action_text': _("View Partners"),
                'action': suppliers._get_records_action(),
            }
        ...
        if invalid_moves := edi_moves.filtered(lambda move: move.invoice_date > fields.Date.context_today(self.with_context(tz='Asia/Riyadh'))):
            res['l10n_sa_edi_invalid_date_moves'] = {
                'message': _("Please set the Invoice Date to be either less than or equal to today as per the Asia/Riyadh time zone, since ZATCA does not allow future-dated invoicing."),
                'level': 'danger',
                'action_text': _("View Invoices"),
                'action': invalid_moves._get_records_action(),
            }
    ...
    def _is_sa_edi_testing_applicable(self, move):
        return move._l10n_sa_is_phase_2_applicable() and move.company_id.l10n_sa_api_mode != 'prod'
    ...
    def _get_all_extra_edis(self) -> dict:
        ...
        res.update({'sa_edi_test': {'label': _("To ZATCA (Testing)"), 'is_applicable': self._is_sa_edi_testing_applicable}})
        return res
