...
comment-->1000.0</TaxExclusiveAmount></xpath>"""
    ...
    def test_import_discount_2(self):
        """
        Some product have no price, but have charges and discount.
        ...
        my_invoice_raw = self.env['account.edi.xml.ubl_bis3']._export_invoice(invoice)[0]
        my_invoice_root = etree.fromstring(my_invoice_raw)
        modifying_xpath = """<xpath expr="(//*[local-name()='InvoiceLine']/*[local-name()='LineExtensionAmount'])" position="replace">
                <LineExtensionAmount currencyID="EUR">1.50</LineExtensionAmount>
            </xpath>
        ...
        imported_invoice = self._import_invoice_as_attachment_on(attachment=xml_attachment, journal=self.company_data["default_journal_sale"])
        self.assertEqual(len(imported_invoice.invoice_line_ids), 1)
        self.assertRecordValues(imported_invoice, [{'amount_total': 1.50}])
        self.assertRecordValues(imported_invoice.invoice_line_ids, [{
