...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _retrieve_allowance_charge_tax(self, company_id, allowance_charge_tax_vals):
        ...
        5. For every combination:
           a. Apply `amount * sign`, prioritizing:
               - Positive amounts for charges.
               - Negative amounts for allowances.
           b. If no match is found, retry with the opposite sign to handle edge cases
               (e.g., negative quantities such as in `test_invoice_with_fixed_tax_on_negative_line`).
        """
        ...
        sign = 1 if is_charge else -1
        for domain in domains:
            for price_include in (False, True):
                for amount in (amount * sign, amount * -sign):
                    tax_domain = domain + Domain([('price_include', '=', price_include), ('amount', '=', amount)])
    ...
    def _retrieve_line_allowance_charges(self, record, line_values, taxes):
        ...
        if all(tax for _, tax in tax_map):
            ...
            for val, _ in tax_map:
                ...
                line_vals.append([
                    val['reason_code'] + " " + val['reason'],  # name
                    1,                                         # quantity
                    sign * val['amount'],                      # price_unit
                    percent_taxes,                             # tax_ids
                ])
