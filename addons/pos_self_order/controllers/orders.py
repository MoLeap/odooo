...
class PosSelfOrderAutoCompleteController(AutoCompleteController):
    ...
    def _get_api_key(self, use_employees_key):
        ...
        if isinstance(referrer, bytes):
            referrer = referrer.decode('utf-8', errors='replace')
...
class PosSelfOrderController(http.Controller):
    ...
    def process_order(self, order, access_token, table_identifier, device_type):
        ...
        # Ensure the delivery fee line is present or absent based on the preset configuration
        if preset_id and preset_id.service_at == 'delivery':
            self._ensure_delivery_fee(order_ids, preset_id)
            order_ids._update_amounts()
    ...
    def _ensure_delivery_fee(self, order, preset):
        ...
        delivery_product_tmpl = preset.delivery_product_id
        if not delivery_product_tmpl:
            return
        delivery_variant = delivery_product_tmpl.product_variant_ids[:1]
        if not delivery_variant:
            return

        non_delivery_lines = order.lines.filtered(
            lambda l: l.product_id.product_tmpl_id != delivery_product_tmpl
        )
        non_delivery_total = sum(l.price_subtotal_incl for l in non_delivery_lines)
        ...
        if delivery_is_free:
            ...
            price = preset.delivery_product_price
            order.env['pos.order.line'].sudo().create({
                'order_id': order.id,
                'product_id': delivery_variant.id,
                'qty': 1,
                'price_unit': price,
    ...
    def _check_delivery_address_for_partner(self, preset, partner):
        ...
        max_distance = preset.delivery_max_distance_km or 0
