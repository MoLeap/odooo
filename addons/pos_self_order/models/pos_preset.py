...
class PosPreset(models.Model):
    ...
    delivery_distance_unit = fields.Char(
        string="Distance unit",
        compute="_compute_delivery_distance_unit",
    )
    ...
    @api.depends('delivery_from_country_id')
    def _compute_delivery_distance_unit(self):
        company_country_code = self.env.company.country_id.code
        for preset in self:
            country_code = preset.delivery_from_country_id.code or company_country_code
            preset.delivery_distance_unit = 'mi' if country_code in _IMPERIAL_COUNTRY_CODES else 'km'

    @api.model
    def default_get(self, fields):
        defaults = super().default_get(fields)
        if defaults.get('delivery_from_address'):
            return defaults
        address_fields = {'delivery_from_address', 'delivery_from_city', 'delivery_from_zip',
                          'delivery_from_country_id', 'delivery_from_state_id'}
        if not address_fields.intersection(fields):
            return defaults
        ...
        if result is None:
            return defaults
        defaults['delivery_from_address'] = company.street
