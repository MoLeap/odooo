...
class PosConfig(models.Model):
    ...
    def _can_use_cash_payment_method(self, cash_method):
        if self.self_ordering_mode == 'kiosk' or not cash_method.config_ids.filtered(lambda config: config != self and config.self_ordering_mode != 'kiosk'):
            return True
        return super()._can_use_cash_payment_method(cash_method)
