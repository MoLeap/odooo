...
class ProductTemplate(models.Model):
    ...
    def create_product_variant(self, product_template_attribute_value_ids):
        ...
        if not combination:
            # Frontend actions from /shop can request variant creation without
            # explicit ptav ids. In that case, use the first possible
            # combination so the variant is created only on user action.
