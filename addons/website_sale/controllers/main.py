...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    def shop(self, page=0, category=None, search="", min_price=0.0, max_price=0.0, tags="", **post):
        ...
        if website.shop_split_variants:
            # In split mode, paginate by variant tiles, not templates.
            variant_count_data = Product._read_group(
                [("product_tmpl_id", "in", search_product.ids)],
                groupby=["product_tmpl_id"],
                aggregates=["__count"],
            )
            variant_counts = {tmpl.id: count for tmpl, count in variant_count_data}
            ...
            # Walk ordered templates to find which contribute to this page.
            page_tmpl_ids = []
            cumulative = 0
            variant_skip = 0
            for tmpl_id in search_product.ids:
                count = variant_counts.get(tmpl_id, 0)
                if not count:
                    continue
                if cumulative + count <= offset:
                    cumulative += count
                    continue
                if not page_tmpl_ids:
                    variant_skip = offset - cumulative
                page_tmpl_ids.append(tmpl_id)
                cumulative += count
                if cumulative - offset >= ppg:
                    break

            products = request.env["product.template"].browse(page_tmpl_ids).with_prefetch()
            products.fetch()
        else:
            variant_skip = 0
            pager = website.pager(
        ...
        # map each product to its variants sorted by website_sequence
        ...
        for product in products:
            ...
            if tmpl_variants:
                ...
                # Dynamic variants may not exist yet — create on demand
                first_variant_id = product._get_first_possible_variant_id()
                variant = (
                    Product.sudo().browse(first_variant_id)
                    if first_variant_id
                    else Product.browse([])
                )
                product_variants[product] = variant
                if first_variant_id:
                    variants_by_tmpl[product.id] = [variant]
        ...
        if website.shop_split_variants:
            # Build ordered variant list, trim to page boundaries.
            page_variants_list = []
            for product in products:
                page_variants_list.extend(variants_by_tmpl.get(product.id, []))
            page_variants_list = page_variants_list[variant_skip : variant_skip + ppg]
            displayed_variant_ids = [v.id for v in page_variants_list]
        else:
            displayed_variant_ids = [
                variants[0].id for variants in variants_by_tmpl.values() if variants
            ]

        displayed_variants = Product.browse(displayed_variant_ids)
        ...
        if website.shop_split_variants:
            split_items = [
                ...
                for variant in page_variants_list
            ]
        ...
        values = {
            ...
            "get_product_prices": lambda variant: variant_prices.get(variant.id, {}),
    ...
    def change_product_config(self, product_id=None, variant_id=None, **options):
        ...
        if sequence in ("top", "bottom", "up", "down"):
            getattr(record, f"set_sequence_{sequence}")()
        if {"x", "y"} <= set(options):
