...
export class IndexedDB {
     ...
     * @param {Function} searchFcn The function used to match records (async).
     * @param {Number} limit The maximum number of results to return.
     * @returns {Promise<any>} The matching records.
     */
    async search(table, searchFcn, limit = 8) {
        this._tables.add(table);
        return this.mutex.exec(() =>
            this._execute((db) => {
                if (db) {
                    return this._search(db, table, searchFcn, limit);
                }
            })
        );
    }
    ...
    async _search(db, table, searchFcn, limit) {
        const results = [];
        let query = null; // Start with no range (fetch from the beginig)
        ...
        while (true) {
            ...
            for (const record of records) {
                if (await searchFcn(record.value)) {
                    results.push(record);
