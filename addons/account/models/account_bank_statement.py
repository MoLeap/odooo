...
class AccountBankStatement(models.Model):
    ...
    def _compute_balance_start(self):
        statement_per_journal = dict(self.env['account.bank.statement']._read_group(
            domain=[('state', '=', 'posted')],
            groupby=['journal_id'],
        ...
        for stmt in self.sorted(lambda x: x.first_line_index or '0'):
            ...
            if stmt.journal_type == 'cash':
                statements = statement_per_journal.get(stmt.journal_id, self.env['account.bank.statement']).sorted(key=lambda s: (s.date, s.id), reverse=True)
                balance_start = statements[0].balance_end_real if statements else 0
            else:
