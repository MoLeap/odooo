...
class ProductProduct(models.Model):
    ...
    def _retrieve_products_batched(self, list_of_product_vals, company=None, extra_domain=None):
        ...
        for product_vals in split_every(1000, list_of_product_vals):
            domains = self._get_products_domain_batched(product_vals)
            ...
            for company_domain in (
                ...
                for domain in domains:
                    if products := self.env['product.product'].search_fetch(
                        expression.AND([domain, company_domain, extra_domain or []]),
                        ['id', 'barcode', 'default_code', 'name']
                    ):
                        lookup = {}
                        for product in products.sorted(lambda p: p.company_id.id or 0):
                            if product.barcode:
                                lookup['barcode', product.barcode] = product
                            if product.default_code:
                                lookup['default_code', product.default_code] = product
                            if product.name:
                                lookup['name', product.name.lower()] = product

                        for val_dict in product_vals:

                            barcode = val_dict.get('barcode')
                            code = val_dict.get('default_code')
                            raw_name = (val_dict.get('name') or '').split('\n', 1)[0]

                            if barcode and (match := lookup.get(('barcode', barcode))):
                                all_products[barcode] = match
                            elif code and (match := lookup.get(('default_code', code))):
                                all_products[code] = match
                            elif raw_name and (match := lookup.get(('name', raw_name.lower()))):
                                all_products[raw_name] = match
