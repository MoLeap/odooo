...
class HrEmployee(models.Model):
    ...
    def is_onboarding(self, company_ids):
        """ Called from the HrEmployeeActionHelper js class
            :param company_ids: the active companies ids
        """
        main_company = self.env.ref("base.main_company").id
        return (
            main_company in company_ids
            and not self.env['hr.employee'].search_count([('company_id', 'in', company_ids)])
        )
