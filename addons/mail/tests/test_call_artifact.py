...
class TestMailCallArtifact(TransactionCase):
    ...
    def test_discuss_artifact_overlap(self):
        ...
        self.env['mail.call.artifact'].create({'discuss_call_history_id': self.call.id, 'start_ms': 2600, 'end_ms': 3600})

        # 3. Significant overlap -> Error
        with self.assertRaises(ValidationError):
            self.env['mail.call.artifact'].create({'discuss_call_history_id': self.call.id, 'start_ms': 3000, 'end_ms': 4000})
    ...
    def test_user_access_member(self):
        ...
        # Check Search Access
        search_res = self.env['mail.call.artifact'].with_user(self.user_member).search([('id', '=', self.artifact.id)])
        ...
        with self.assertRaises(AccessError):
            self.env['mail.call.artifact'].with_user(self.user_member).create({
    ...
    def test_user_access_non_member(self):
        ...
        # Check Search Access
        search_res = self.env['mail.call.artifact'].with_user(self.user_non_member).search([('id', '=', self.artifact.id)])
    ...
    def test_admin_access(self):
        ...
        # Check Create Access
        new_artifact = self.env['mail.call.artifact'].with_user(self.admin_user).create({
            'discuss_call_history_id': self.call.id,
            'start_ms': 5000,
            'end_ms': 6000,
        })
