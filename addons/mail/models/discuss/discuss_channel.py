...
class DiscussChannel(models.Model):
    ...
    @api.constrains('group_public_id', 'auto_join')
    def _constraint_group_id_channel(self):
        # sudo: discuss.channel - skipping ACL for constraint, more performant and no sensitive information is leaked
        failing_channels = self.sudo().filtered(
            lambda channel: channel.channel_type != 'channel' and (
                channel.group_public_id or channel.auto_join
            ),
        )
        if failing_channels:
            raise ValidationError(_("For %(channels)s, channel_type should be 'channel' to have the group-based authorization or auto-join.", channels=', '.join([ch.name for ch in failing_channels])))
