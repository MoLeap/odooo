...
class SaleReport(models.Model):
    ...
    def _select_pos(self):
        select_ = f"""
            ...
            AVG(l.price_unit) AS price_unit,
            SUM(SIGN(l.qty) * SIGN(l.price_unit) * ABS(l.price_subtotal_incl)) AS price_total,
            SUM(SIGN(l.qty) * SIGN(l.price_unit) * ABS(l.price_subtotal)) AS price_subtotal,
            CASE WHEN pos.account_move IS NULL THEN SUM(l.price_subtotal) ELSE 0 END AS amount_to_invoice,
            CASE WHEN pos.account_move IS NOT NULL THEN SUM(l.price_subtotal) ELSE 0 END AS amount_invoiced,
            CASE WHEN pos.account_move IS NOT NULL THEN SUM(l.price_unit * l.qty_delivered) ELSE 0 END AS untaxed_delivered_amount,
            count(*) AS nbr,
            pos.name AS name,
            ...
            SUM(l.price_unit * l.discount * l.qty / 100.0) AS discount_amount,
