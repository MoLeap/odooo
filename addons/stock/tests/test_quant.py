...
class TestStockQuant(TestStockCommon):
    ...
    def test_request_quant_relocation(self):
        ...
        wizard.action_request_quant_relocation()
        picking = self.env['stock.picking'].search([('origin', '=', 'Relocation Request: Product needs relocation')])
        self.assertTrue(picking)

        # Multi-location quant relocation request with common internal parent
        wizard = self.env['stock.quant.relocate'].create({
        ...
        # Multi-location quat relocation request with no common internal parent
        wizard = self.env['stock.quant.relocate'].create({
