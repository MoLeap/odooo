...
class PosSession(models.Model):
    ...
    def _search_move_ids(self, operator, value):
        ...
        pickings = self.env['stock.picking'].search([('move_ids.account_move_id', 'in', moves.ids)])
        return [
            '|',
    ...
    @api.depends('sales_move_id', 'refunds_move_id', 'picking_ids.move_ids.account_move_id')
    def _compute_move_ids(self):
        for session in self:
            stock_account_moves = session.picking_ids.move_ids.account_move_id
            session.move_ids = session.sales_move_id | session.refunds_move_id | stock_account_moves
    ...
    def _handle_starting_cash_balance(self, cashbox_value=0):
        self.ensure_one()
        if cash_pm := self.sudo()._get_cash_payment_method():
    ...
    def _create_session_account_move(self, orders, refund=False):
        ...
        # stock valuation commands for real-time valued products on non-invoiced orders
        stock_valuation_commands = orders._prepare_stock_valuation_line_commands()
        ...
        move_ctx.with_company(self.company_id).write({
            ...
            'line_ids': payment_commands + stock_valuation_commands,
        })
        move_ctx._post()
