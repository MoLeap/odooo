...
class PosOrder(models.Model):
    ...
    def _prepare_account_move_line_data_for_payments(self, partner=None):
        ...
        for payment in self.payment_ids:
            ...
            if payment.pos_order_id.is_invoiced:
                continue
    ...
    def _prepare_stock_valuation_line_commands(self):
        """
        Build Command.create entries for stock valuation journal lines
        (DR Expense / CR Stock Valuation) for non-invoiced orders whose
        products use real-time inventory valuation.
        """
        commands = []
        if self.session_id.update_stock_at_closing:
            # Pickings are created AFTER session accounting for this mode;
            # stock valuation for that case is handled separately.
            return commands

        # Collect all pickings from non-invoiced, non-shipping-date orders
        picking_ids = self.filtered(
            lambda o: not o.shipping_date,
        ).picking_ids
        ...
        for move in stock_moves:
            # Skip if the location already has a dedicated valuation account
            # (those are handled by stock_account's own _create_account_move).
            if move.location_id.valuation_account_id or move.location_dest_id.valuation_account_id:
                continue

            product_ctx = move.with_company(move.company_id).product_id
            ...
            value = move.value  # positive for out moves (sales), positive for in (returns)
            label = _("Stock variation for %s", move.product_id.display_name)
            if move.is_out:
                # Sale: DR Expense, CR Stock Valuation
                commands.extend([
                    Command.create({
                        'name': label,
                ...
                # Return: DR Stock Valuation, CR Expense
                commands.extend([
                    Command.create({
                        'name': label,
                        'account_id': stock_valuation_account.id,
    ...
    def _create_payment_moves(self, session, payments, partner=None):
        ...
        for pm_data in payments:
            ...
            if pm.type == 'cash':
                all_payment_lines |= pm._create_cash_payment_line(
                    session,
                    amount,
                    partner,
                )
            elif pm.type == 'bank':
                all_payment_lines |= pm._create_bank_payment_line(
                    session,
                    amount,
                    partner,
                )
    ...
    def _prepare_invoice_vals(self):
        ...
        sales, refund = self._prepare_account_move_line_data(False)
        line_commands = [Command.create(line) for line in sales]
        line_commands += [Command.create(line) for line in refund]
    ...
    def _generate_pos_order_invoice(self):
        ...
        payments = self._prepare_account_move_line_data_for_payments(
            self.partner_id,
        )
        ...
        if self.config_id.cash_rounding and invoice.invoice_cash_rounding_id:
            ...
            if not currency.is_zero(difference_curr):
                ...
                diff = existing_terms_line.amount_currency - difference_curr
    ...
    def _generate_invoice_after_session_closing(self):
        ...
        global_move = refund_move if self.is_refund else sale_move
        if not global_move or global_move.state != "posted":
            return self.env['account.move']

        invoice = self._generate_pos_order_invoice()
        self._create_partial_reversal_move_from_session_closing(invoice)
        return invoice

    def _create_partial_reversal_move_from_session_closing(self, invoice_to_reverse):
        """
        ...
        reverse_moves = []
        session = self.session_id
        session_receivable_account = session._get_receivable_account()
        ...
        if self.picking_ids.ids:
            ...
            for stock_move in stock_moves:
                ...
                reverse_moves.append(Command.create({
        ...
        for line in product_lines + tax_lines:
            reverse_moves.append(Command.create({
                'name': line.name,
                'account_id': session_receivable_account.id,
        ...
        # Pick corresponding payment amount from global move to reverse
        # it as well in the same move
        payment_lines = invoice_to_reverse.line_ids.filtered(
            lambda line: line.display_type == 'payment_term',
        )
        for payment in payment_lines:
            reverse_name = _("Reversal of payment: %s", payment.name)
            ...
            reverse_moves.append(receivable_line)

        Move = self.env['account.move'].sudo().with_company(self.company_id)
        move_ctx = Move.with_context(
            linked_to_pos=True,
        )
        reversal_move = move_ctx.create({
            'date': fields.Date.today(),
            'ref': _("Partial reversal of %s", invoice_to_reverse.name),
            'line_ids': reverse_moves,
            'journal_id': self.config_id.journal_id.id,
        })
        reversal_move._post()
