...
export class PosWebrtcService {
    ...
    async establishConnection(targetPeerId) {
        ...
        this.createPeerConnection(targetPeerId);
        const peer = this.peers.get(targetPeerId);
        if (!peer || !peer.peerConnection) {
            return;
        }

        peer.channel = peer.peerConnection.createDataChannel("notifications");
        this.attachChannelHandlers(targetPeerId, peer.channel, "send");
    ...
    async handleCandidate(candidate) {
        const peerId = candidate.fromPeerId || "legacy";
        ...
        if (!peer || !peer.peerConnection) {
            ...
            logPosMessage(
                "PosWebrtcService",
                "handleCandidate",
                "no peerconnection for candidate",
                CONSOLE_COLOR
            );
            return;
        }
    ...
    async handleReady(data) {
        const peerId = data.fromPeerId || "legacy";
        this.knownPeers.add(peerId);
        if (this.peers.has(peerId)) {
            return;
        }
        if (!data.fromPeerId && this.peers.size) {
            return;
        }
    ...
    async handleOffer(offer) {
        const peerId = offer.fromPeerId || "legacy";
        if (this.peers.has(peerId)) {
            logPosMessage(
                "PosWebrtcService",
                "handleOffer",
    ...
    async handleAnswer(answer) {
        const peerId = answer.fromPeerId || "legacy";
        const peer = this.peers.get(peerId);
        if (!peer || !peer.peerConnection) {
            logPosMessage(
                "PosWebrtcService",
    ...
    onDataChannelReceived(peerId, event) {
        ...
        this.attachChannelHandlers(peerId, peer.channel, "receive");
    }
    ...
    createPeerConnection(peerId) {
        ...
        peerConnection.onicecandidate = (e) => this._onIceCandidate(e, peerId);
        peerConnection.onconnectionstatechange = () => this._onConnectionStateChange(peerId);
        peerConnection.oniceconnectionstatechange = () => this._onIceConnectionStateChange(peerId);
    }

    _onIceCandidate(e, peerId) {
        const message = {
            type: "candidate",
            fromPeerId: this.peerId,
            toPeerId: peerId,
    ...
    _onConnectionStateChange(peerId) {
        const peer = this.peers.get(peerId);
    ...
    _onIceConnectionStateChange(peerId) {
        const peer = this.peers.get(peerId);
        if (!peer || peer.isClosing) {
            return;
        }
    ...
    attachChannelHandlers(peerId, channel, direction) {
        channel.onopen = () => this._onChannelStateChange(peerId, channel, direction);
        channel.onclose = () => this._onChannelStateChange(peerId, channel, direction);
        channel.onmessage = (event) => this._onChannelMessageCallback(peerId, event);
        channel.onerror = (event) => this._onChannelError(peerId, event);
    }

    _onChannelStateChange(peerId, channel, direction) {
        const peer = this.peers.get(peerId);
        if (!peer || peer.isClosing) {
            return;
        }
        ...
        logPosMessage(
            "PosWebrtcService",
            "ChannelState",
            `Channel ${direction} ${readyState} for peer ${peerId}`,
            CONSOLE_COLOR
        );
    }

    _onChannelMessageCallback(peerId, event) {
        for (const listener of this.listeners) {
            listener(event.data);
        }
    }

    _onChannelError(peerId, event) {
        logPosMessage(
            "PosWebrtcService",
            "ChannelError",
            `Channel error for peer ${peerId}`,
            CONSOLE_COLOR,
            [event]
        );
        this.removePeer(peerId);
    ...
    startConnectionTimeout(peerId) {
        ...
        const timer = setTimeout(() => {
            const peer = this.peers.get(peerId);
            if (!peer || !this.isPeerConnected(peer)) {
                this.removePeer(peerId);
            }
