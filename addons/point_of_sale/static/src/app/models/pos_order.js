...
export class PosOrder extends PosOrderAccounting {
    ...
    updateServiceCharge() {
        ...
        try {
            ...
            if (preset.service_fee_type === "fixed") {
                price = isNewFixedLine ? feeAmount : line.price_unit;
    ...
    getPreDiscountTotal() {
        const basicOrderLines = this.lines.filter((l) => !l.isSpecialLine);
    ...
    getPostDiscountTotal() {
        const nonServiceChargeLines = this.lines.filter((l) => !l.is_service_charge);
