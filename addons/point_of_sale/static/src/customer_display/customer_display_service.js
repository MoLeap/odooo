...
export class CustomerDisplayService {
     ...
     *
     * Override this method to enrich or customize the data derived
     * from the order before it is transmitted (e.g., add extra fields,
     * metadata, or UI-specific flags).
     *
    ...
    sendOrder(order) {
        if (!this.models || !this.GeneratePrinterData || !order) {
            return;
        }
        this.send(this._buildDisplayPayload(order));
    }
