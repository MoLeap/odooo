...
export function postMessage(message, description = "") {
    return run(() => {
        posmodel._onDataReceived(typeof message !== "string" ? JSON.stringify(message) : message);
...
export const getScaleDataJson = (payload = {}) => {
    const {
        productName = "Weighing Product",
        grossWeight = 1,
        tare = 0,
...
export function startCustomerDisplay() {
    return {
        ...
        run: async () => postMessage(initOrderData).run(),
    };
}
...
export const ADD_PRODUCT_SELECTED = JSON.stringify(
    getOrderJsonData({
        lines: [getLineJsonData({ isSelected: true })],
    })
);
...
export const UPDATE_SCALE = JSON.stringify(
    getOrderJsonData({
        scaleData: getScaleDataJson({
            productName: "Apple",
            grossWeight: 2,
            tare: 0.2,
            unitPrice: 5,
...
export const PAY_ONLINE = (() => {
    const lines = [getLineJsonData({ isSelected: true })];
