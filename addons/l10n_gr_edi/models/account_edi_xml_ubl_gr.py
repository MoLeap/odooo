...
class AccountEdiXmlUblGr(models.AbstractModel):
    ...
    def _format_greek_invoice_number(self, invoice):
        ...
        company_vat = company_vat[2:] if company_vat.startswith('EL') else company_vat
    ...
    def _add_invoice_line_item_nodes(self, line_node, vals):
        ...
        if not line or not line.product_id or not line.product_id.l10n_gr_edi_cpv_code:
            return
        item = line_node.setdefault('cac:Item', {})
        commodity_node = item.setdefault('cac:CommodityClassification', [])
        commodity_node.append({
            'cbc:ItemClassificationCode': {
                '_text': line.product_id.l10n_gr_edi_cpv_code,
                'listID': 'CPV'
            }
        })
    ...
    def _validate_greek_business_rules(self, invoice):
        ...
        checks = [
            ...
            ('gr_r_003', not vat.startswith('EL'), _("Supplier VAT must start with 'EL'")),
            ('gr_r_004_1', not invoice.l10n_gr_edi_mark, _("M.AR.K number is required for Suppliers")),
            ('gr_r_006', not buyer_vat or not buyer_vat.startswith('EL'), _("Buyer VAT must start with prefix 'EL'")),
            ('gr_bt_10', not invoice.partner_id.l10n_gr_edi_contracting_authority_name, _("Contracting authority name is required for Greek buyer")),
            ('gr_bt_46', not invoice.partner_id.l10n_gr_edi_contracting_authority_code, _("Contracting authority code is required for Greek buyer")),
            ('gr_r_007', not (invoice.l10n_gr_edi_budget_type and invoice.l10n_gr_edi_project_reference), _("Budget Type and Project reference is required for B2G invoicing")),
        ]
