...
class AccountMove(models.Model):
    ...
    def _compute_l10n_es_edi_sii_data(self):
        for move in self:
            ...
            accepted_docs = docs.filtered(lambda d: d.state in ('accepted', 'accepted_with_errors')).sorted('create_date', reverse=True)
            move.l10n_es_edi_csv = accepted_docs[0].csv if accepted_docs else False

            latest_doc = docs.sorted('create_date', reverse=True)[0]
    ...
    def button_cancel(self):
        for move in self.filtered(lambda m: m.l10n_es_edi_sii_state == 'sent'):
            if not move._send_l10n_es_sii_document(cancel=True) or move.l10n_es_edi_sii_state != 'cancelled':
