...
class EditableButtonPlugin extends Plugin {
    ...
    resources = {
        ...
        clean_for_save_handlers: this.cleanEditableButtons.bind(this),
    };
    ...
    wrapEditableButtons(root) {
        const editableButtons = selectElements(root, "button.o_savable[contenteditable='true']");
        for (const button of editableButtons) {
            button.removeAttribute("contenteditable");

            if (
                button.childNodes.length === 1 &&
                button.querySelector(":scope > .o_inner_button_editable_span")
            ) {
                continue;
            }
