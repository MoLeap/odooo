...
registerWebsitePreviewTour(
    ...
    () => [
        ...
        },
        {
            ...
            async run({ check, waitFor }) {
                // view_rendering_delay
                await check();
                const trigger = `.o_list_renderer ${homePage} td.o_list_record_selector input[type="checkbox"]`;
                for (let i = 0; i < 10; i++) {
                    try {
                        await waitFor(`${trigger}:checked`, { timeout: 500 });
                        break;
                    } catch {
                        await check(trigger);
                    }
