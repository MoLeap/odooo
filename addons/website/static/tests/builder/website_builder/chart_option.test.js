...
test("Adding / Deleting multiple columns updates the data and available cells", async () => {
    ...
    await contains(":iframe .s_chart").click();
    await openChartSlidingPanel();
    await contains(".hb-sliding-panel [data-action-id=setColumn] input").edit(10);
    data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
    ...
    await contains(".hb-sliding-panel [data-action-id=setColumn] input").edit(1);
    data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
    expect(data.datasets).toHaveLength(1);
    expect(".hb-sliding-panel table thead tr th").toHaveCount(3);
    expect(".hb-sliding-panel table tbody tr:first td").toHaveCount(2);
...
test("Adding / Deleting multiple rows updates the data and available cells", async () => {
    ...
    await contains(":iframe .s_chart").click();
    await openChartSlidingPanel();
    await contains(".hb-sliding-panel [data-action-id=setRow] input").edit(10);
    data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
    ...
    await contains(".hb-sliding-panel [data-action-id=setRow] input").edit(1);
    data = JSON.parse(queryFirst(":iframe .s_chart").dataset.data);
...
test("Pasting values update multiple chart values at once", async () => {
    const type = "bar";
    const { waitSidebarUpdated } = await setupWebsiteBuilder(chartTemplate(type, getData(type)));
    await contains(":iframe .s_chart").click();
    await openChartSlidingPanel();
    await contains(".hb-sliding-panel [data-action-id=setColumn] input").edit(5);
    await contains(".hb-sliding-panel [data-action-id=setRow] input").edit(5);
