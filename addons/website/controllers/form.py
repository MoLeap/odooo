...
class WebsiteAuthSignupHome(AuthSignupHome):
    ...
    def web_auth_signup(self, *args, **kwargs):
        ...
        Custom fields (not in res.users model) are logged as chatter messages on
        ...
        # Prepare and log message for fields not in res.users model.
    ...
    def _prepare_signup_values(self, qcontext):
        """
        Extend the signup value preparation to:
        - Validate that the password and confirm password match. If they do not
        match, raise a UserError with a dict containing the field to highlight
        in the form and the error message.
        ...
        :raises UserError: if the passwords do not match or a non-image file is
                        provided for an image field
        """
