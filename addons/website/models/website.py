...
class Website(models.Model):
    ...
    def _control_third_party_trackers_in_html(self, html):
        if not html or not self._should_remove_third_party_trackers():
            return html
        try:
            root_node = lxml.html.fromstring(str(html))
            tags = root_node.xpath("//script | //iframe")
        except (ParserError, XMLSyntaxError):
            return html
        for tag in tags:
            self._remove_third_party_trackers(tag.tag, tag.attrib, ['domains'])
        return Markup(lxml.html.tostring(root_node, encoding="unicode"))
