...
test.tags("desktop");
test("form: closing overtime details without changes does not write", async () => {
...
test.tags("desktop");
test("form: overtime details writes edited manual duration", async () => {
    onRpc("hr.attendance.overtime.line", "write", ({ args }) => {
        expect.step(`${args[0][0]}:${args[1].manual_duration}`);
    });
    await mountAttendanceForm(1);
    await openOvertimePopover();
    await contains(".popover tbody tr:first-child td:nth-child(2) input").edit(
        "02:00",
    );
    await animationFrame();
    await contains(".popover .btn-primary").click();
    await expect.waitForSteps(["11:2"]);
});

test.tags("desktop");
test("form: approved overtime details stay readonly", async () => {
    await mountAttendanceForm(2);
    await openOvertimePopover();
    expect(".popover .popover-body tbody tr").toHaveCount(1);
    expect(".popover tbody tr:first-child td:nth-child(2) input").toHaveCount(0);
    expect(".popover tbody tr:first-child td:nth-child(3) input").toHaveCount(0);
});
