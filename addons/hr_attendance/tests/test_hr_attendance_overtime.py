...
class TestHrAttendanceOvertime(HttpCase):
    ...
    def test_auto_check_out_specific_time(self):
        ...
        night_shift, before_cutoff, one_min_before, midnight, just_after_midnight, already_checked, flexible = self.env['hr.attendance'].create([
            ...
            {'employee_id': self.jpn_employee.id, 'check_in': datetime(2026, 2, 26, 4, 59)},
            {'employee_id': self.honolulu_employee.id, 'check_in': datetime(2026, 2, 25, 23, 0)},
        ...
        self.assertEqual(night_shift.check_out, datetime(2026, 2, 26, 5, 0))
        self.assertEqual(night_shift.out_mode, 'auto_check_out')
        self.assertEqual(before_cutoff.check_out, datetime(2026, 2, 26, 5, 0))
        self.assertEqual(one_min_before.check_out, datetime(2026, 2, 26, 5, 0))
        self.assertEqual(midnight.check_out, datetime(2026, 2, 26, 5, 0))
        self.assertEqual(just_after_midnight.check_out, datetime(2026, 2, 26, 5, 0))
        self.assertEqual(already_checked.check_out, initial_checkout)
        self.assertEqual(flexible.check_out, datetime(2026, 2, 26, 5, 0))

    @freeze_time("2026-02-27 07:00:00")
    def test_auto_check_out_specific_time_after_cutoff(self):
        """Test check-in after or exactly at cutoff"""
        self.company.write({
            'auto_check_out': True,
            'auto_check_out_mode': 'specific_time',
            'auto_check_out_specific_time': 6.0,
        })
        after_cutoff, exactly_at_cutoff, minimum_duration = self.env['hr.attendance'].create([
            {'employee_id': self.employee.id, 'check_in': datetime(2026, 2, 26, 6, 0)},
            {'employee_id': self.other_employee.id, 'check_in': datetime(2026, 2, 26, 5, 0)},
            {'employee_id': self.jpn_employee.id, 'check_in': datetime(2026, 2, 26, 5, 0)},
        ])
        self.env['hr.attendance']._cron_auto_check_out_specific_time()
        self.assertEqual(after_cutoff.check_out, datetime(2026, 2, 27, 5, 0))
        self.assertEqual(exactly_at_cutoff.check_out, datetime(2026, 2, 27, 5, 0))
        self.assertTrue(minimum_duration.check_out)
        self.assertGreater(minimum_duration.check_out, minimum_duration.check_in)
    ...
    def test_auto_check_out_specific_time_old_attendances(self):
        ...
        multiday, backdated, very_old = self.env['hr.attendance'].create([
            ...
            {'employee_id': self.jpn_employee.id, 'check_in': datetime(2026, 2, 26, 1, 0)},
        ])
        self.env['hr.attendance']._cron_auto_check_out_specific_time()
        self.assertEqual(multiday.check_out, datetime(2026, 2, 24, 5, 0))
        self.assertEqual(backdated.check_out, datetime(2026, 2, 25, 5, 0))
        self.assertEqual(very_old.check_out, datetime(2026, 2, 26, 5, 0))

    @freeze_time("2026-02-26 13:00:00")
    def test_auto_check_out_specific_time_multicompany_timezone(self):
        """Test multiple companies with different timezones and cutoff times"""
        self.company.write({
            'auto_check_out': True,
            'auto_check_out_mode': 'specific_time',
            'auto_check_out_specific_time': 6.0,
        ...
        att_main, att_brussels = self.env['hr.attendance'].create([
            {'employee_id': self.employee.id, 'check_in': datetime(2026, 2, 26, 1, 0)},
            {'employee_id': brussels_employee.id, 'check_in': datetime(2026, 2, 26, 7, 0)},
        ])
        self.env['hr.attendance']._cron_auto_check_out_specific_time()
    ...
    def test_auto_check_out_specific_time_edge_times(self):
        ...
        self.env['hr.attendance']._cron_auto_check_out_specific_time()
        self.assertEqual(end_of_day.check_out, datetime(2026, 2, 26, 22, 59))
        with freeze_time("2026-02-26 07:00:00"):
            ...
            self.env['hr.attendance']._cron_auto_check_out_specific_time()
            self.assertEqual(start_of_day.check_out, datetime(2026, 2, 25, 23, 0))
