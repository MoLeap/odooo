...
class HrAttendanceReport(models.Model):
    ...
    def _compute_attendance_metrics(self, report_records):
        ...
        overtime_map = {(ot.employee_id.id, ot.date): ot for ot in overtime_lines}
        ...
        for report_record in report_records:
            ...
            # 3. Overtime Hours
            day_overttime_lines = overtime_map.get((emp.id, c_in.astimezone(employee_tz).date()), self.env['hr.attendance.overtime.line'])
            overtime_hours = sum(day_overttime_lines.mapped('manual_duration'))
            validated_overtime_hours = sum(day_overttime_lines.filtered(lambda ot: ot.status == 'approved').mapped('manual_duration'))
