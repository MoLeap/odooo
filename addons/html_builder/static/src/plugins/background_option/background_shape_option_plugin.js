...
export class BackgroundShapeOptionPlugin extends Plugin {
    ...
    resources = {
        ...
        on_snippet_addition_dialog_handlers: this.onSnippetsSelectionDialogOpen.bind(this),
    ...
    syncShapeColorsWithTheme(updatedColorVariable) {
        ...
        const targetedBackgroundShapeEls = Array.from(this.document.querySelectorAll(selector));
        this.refreshShapeEls(targetedBackgroundShapeEls);
    }
    /**
     ...
     *
     * @param {Object} params
     * @param {HTMLElement} params.snippetModel - The snippet model
     */
    onSnippetsSelectionDialogOpen({ snippetModel }) {
        const customSnippets = snippetModel.snippetsByCategory.snippet_custom;
        if (!customSnippets.length) {
            return;
        }
        const selector = `[data-oe-shape-data*="o-color-"] .o_we_shape[style*="background-image"]`;
        const targetedBackgroundShapeEls = customSnippets.flatMap((snippet) =>
            Array.from(snippet.content.querySelectorAll(selector))
        );
        this.refreshShapeEls(targetedBackgroundShapeEls);
    }
    refreshShapeEls(shapeEls) {
