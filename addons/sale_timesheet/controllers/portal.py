...
class SaleTimesheetSaleCustomerPortal(CustomerPortal):
    ...
    def _sale_order_get_page_view_values(self, order_sudo, access_token, values, history_session_key, **kwargs):
        values = self._get_page_view_values(order_sudo, access_token, values, history_session_key, False, **kwargs)
        ...
        return {
            **values,
            "is_timesheet": request.env['account.analytic.line'].sudo().search_count(domain, limit=1)
        }
