...
class TestWebsocketDispatching(WebsocketCase):
    ...
    def test_replay_notifications_for_new_channels(self, clock):
        client = self.websocket_connect()
        initial_last_id = self.env["bus.bus"]._bus_last_id()
        self.subscribe(client, ['channel_A'], last=initial_last_id)
        self.env['bus.bus']._sendone('channel_B', 'channel_b_notification', None)
        self.env['bus.bus']._sendone('channel_A', 'channel_a_notification', None)
        self.trigger_notification_dispatching(['channel_A'])
        self.assertEqual(len(json.loads(client.recv())), 1)
        # Make sure the last_id kept by the server advances in order to ensure the server
        ...
        self.trigger_notification_dispatching(['channel_A'])
        self.assertEqual(len(json.loads(client.recv())), 1)
        self.subscribe(client, ['channel_A', 'channel_B'], last=initial_last_id)
    ...
    def test_subscribe_to_new_channel_with_higher_id(self, clock):
        ...
        self.trigger_notification_dispatching(['channel_A'])
        self.assertEqual(len(json.loads(client.recv())), 1)
        # At this point, the last_id for A is equal or below the last notification. B will
        ...
        # The client has an higher id. This is common since the server last_id is held
