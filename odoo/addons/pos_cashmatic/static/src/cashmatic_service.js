import {
    POST_REQUESTS
} from "@pos_cashmatic/utils/constants";
...
export class CashmaticService{
    ...
    async _sendRequest(operation, params) {
        ...
        const url = `${protocol}//${this.ip}${operation}`;
    ...
    async renewOrLogin() {
        try {
            ...
            const response = await this._sendRequest(POST_REQUESTS.login, { username: this.username, password: this.password })
            if (!response?.token) {
                throw new Error("No token");
            }
            this.token = response.token
